...
showHelp() {
    ...
    echo "Error: $1 $2"
    echo " "
    echo "    local.sh [APPLICATION] [COMMAND]"
    echo " "
    ...
    echo " "
    echo "    example: ./local.sh wallet create"
    echo "    example: ./local.sh wallet send"
    echo "    example: ./local.sh wallet update"
    echo "    example: ./local.sh wallet fromkey"
    echo " "
}

calljava() {
    echo "Running the command line with command $COMMAND"
    java -jar -Dlight-4j-config-dir=config/local taiji-console/build/libs/taiji-console-fat-1.0.0.jar $APPLICATION $COMMAND
}
